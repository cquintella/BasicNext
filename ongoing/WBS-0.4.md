...
#### Activity 3.6 — Close native `HOST.Net` resource and timeout gaps

- **Status:** DONE — 2026-09-02.
- **Objective:** Ensure direct host networking operations have bounded resource ownership, configured limits, real deadlines, and consistent address classification.
- **Deliverables:** Resolver timeout tasks retain and later reap `JoinHandle`s; TCP connect applies its deadline to read/write operations; TCP listeners use the requested backlog; accept uses an async timeout without busy polling; TCP/UDP/listener handles share a configured socket quota; resolver result count and UDP datagram size come from the registry; IPv4-mapped addresses follow IPv4 `IsPrivate`/`IsLoopback` rules; UDP multicast, limited broadcast, and directed `.255` destinations are denied.
- **Dependencies:** Activities 0.3, 0.5, 2.3, and 3.1; `src/net.rs`; `src/runtime/executor/part14.rs`; `src/runtime/executor/part15.rs`; `src/runtime/executor/part16.rs`; `src/config.rs`; `config/0.4-bnweb-limits.toml`.
- **Acceptance criteria:** No timed-out resolver task is spawned without a retained join handle; every inserted native socket is subject to the configured quota; TCP connect has bounded connect/read/write behavior; TCPListen honors the supplied backlog; Accept does not spin at a fixed 1 ms interval; no direct network operation bypasses configured resolver/datagram limits; mapped predicates agree with SSRF classification; restricted UDP destinations fail before transport.
- **Verification:** `net::tests::tcp_listener_bind_with_backlog_uses_bounded_socket_queue`, `tcp_listener_accept_timeout_returns_empty`, `tcp_listener_accept_timeout_restores_the_accepted_stream_to_blocking`, `udp_send_rejects_broadcast_and_multicast_destinations`, `config::tests::embedded_registry_has_accepted_defaults`, full `cargo test --all-targets`, `cargo clippy --all-targets -- -D warnings`, and `cargo fmt --check` pass. `Ping`, `Neighbor`, and `Reverse` are tested as deterministic `HOST_CAPABILITY_UNAVAILABLE` provider boundaries and remain outside the accepted native implementation until separately specified.
...
- **Status:** DONE — 2026-09-02 after the BNDispatch recovery matrix passed.
- **Objective:** Restore the bounded async surface so its native provider satisfies the accepted 0.4 lifecycle, capacity, ownership, and isolation contract.
- **Deliverables:** The existing grammar/AST/IR surface plus a repaired `Dispatch.Queue` worker lifecycle; legal ticket transitions; panic-to-failure isolation; non-terminal pending accounting; broken-generation barriers; owner-checked mutexes; capped semaphores; checked groups; a typed `DispatchLimits` snapshot; retained ticket failures; and direct executor-isolation evidence. The approved design is [`docs/superpowers/specs/2026-09-02-bndispatch-recovery-design.md`](../docs/superpowers/specs/2026-09-02-bndispatch-recovery-design.md). LLVM remains explicitly unsupported for async operations.
- **Dependencies:** Activities 0.2 and 0.4; recovery design; `src/config.rs`; `config/0.4-bnweb-limits.toml`; `src/dispatch.rs`; `src/dispatch/sync.rs`; `src/runtime/executor/part3.rs`; `modules/bn/BNDispatch.bn`; and the 0.4 external-module documentation.
- **Acceptance criteria:** Queue Close disconnects and joins all workers before reporting success; pending cancellation prevents user-code entry; panics produce `FAILED` without reducing future service; poisoned locks do not abort the process; self-join from a worker returns `SelfJoin`; only non-terminal tickets consume pending capacity; ticket IDs are opaque and non-sequential; barrier timeouts never yield false success; invalid Unlock/Release/Leave return `Error`; every dispatch threshold comes from `DispatchLimits`; ticket failure survives Close; and async tasks demonstrably do not share heap, output, handles, or PRNG stream. `Mutex.Unlock`, `Semaphore.Release`, and `Group.Leave` return `VOID OR Error` in the 0.4 contract.
- **Verification:** Recovery tests cover real concurrent barrier waiters, worker join/deadline, cancellation-before-start, panic recovery, poisoned-lock recovery, self-join rejection, opaque ticket IDs, sequential capacity reuse, synchronization misuse, retained failure, and task isolation. `runtime::tests::async_tasks_do_not_cross_talk_through_host_random_state` is the BN-DISPATCH-009 regression. `cargo test --all-targets`, `cargo clippy --all-targets -- -D warnings`, and `cargo fmt --check` pass. Scoped `git diff --check` passes; the repository-wide check still reports four trailing-space lines in pre-existing `docs/book` edits outside this activity.
...
- **Status:** REOPENED — 2026-09-02 because BNDispatch Activity 5.1 and Gate G5 are open.
- **Objective:** Re-close 0.4 only when the recovered BNDispatch provider, specification, implementation, security register, conformance evidence, and integrations agree.
- **Deliverables:** Updated final WBS/bucket status, corrected BNDispatch module and language documentation, recovery tests/evidence, release advisory removal only after acceptance, examples, VS Code/Jupyter regression evidence, and residual-risk acceptance.
- **Dependencies:** Reopened Activity 5.1 and Gate G5; all previously accepted activities. Activities 4.3 and 6.5 remain excluded unless separately accepted.
- **Acceptance criteria:** No unresolved authority conflict or unowned high/critical finding remains; BNDispatch satisfies the approved recovery design; all required checks pass; unsupported capabilities are explicit; and plugin versions/examples match the recovered language contract.
- **Verification:** First execute the recovery design's focused matrix, then run `cargo test --all-targets -- --test-threads=1`, `cargo clippy --all-targets -- -D warnings`, `cargo fmt --check`, `git diff --check`, `node plugins/vscode/test/debug-adapter.js`, `node plugins/vscode/test/test.js`, and the Jupyter Program/wire suite. No commit, publish, or release operation is part of this WBS.
