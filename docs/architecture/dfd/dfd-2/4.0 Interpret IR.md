...
**Scope:** decompose **4.0 Interpret IR** only. This path is the **executable reference** (subordinate to the language specification) — LLVM `lli` is not the meaning of “run.” See [`../../conformance.md`](../../conformance.md).
...
flowchart TB
  ...
  P41(("4.1 Bind HostEnv<br/>providers + policy"))
...
1. **4.1 Bind HostEnv** binds **providers** (target support) and installs **execution policy** with scopes (not a single Capabilities boolean). Program requirements come from the IR/module. See [`../../host-traits.md`](../../host-traits.md).
...
| Id | Process |
| --- | --- |
| 4.1 | Bind HostEnv (providers + execution policy) |
| 4.2 | Execute BN IR |
| 4.3 | HOST services |
