...
impl AddressResolver for SystemAddressResolver {
    fn resolve(
        ...
        bn_host_net::net::resolve(host, port, maximum).map_err(|error| error.to_string())
    }
}
...
fn client_request_once(
    ...
    if body.len() > bn_limits::web_limits().max_body_bytes {
    ...
    for address in &addresses {
        let endpoint = match bn_host_net::net::Address::parse(&address.to_string()) {
            Ok(addr) => bn_host_net::net::Endpoint::new(addr, port),
            Err(_) => continue,
        };
        match bn_host_net::net::TcpStream::connect(
            endpoint,
    ...
    I: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    .map_err(|error| error.to_string())?
    .to_bytes();
    if bytes.len() > bn_limits::web_limits().max_response_body_bytes {
...
fn resolve_validated_addresses_with_policy(
    ...
    let addresses = resolver
        .resolve(host, port, bn_limits::web_limits().resolved_addresses_max)?
        .into_iter()
        .map(bn_host_net::net::Address::as_std)
        .collect::<Vec<_>>();
    policy
