...
impl Builder<'_> {
    pub(super) fn short_circuit(
        ...
        let right_result = self.value();
        self.emit(Instruction::Copy {
            destination: right_result,
            source: right,
            ty: Type::Boolean,
            span,
        });
        ...
        self.emit(Instruction::Phi {
            destination,
            incoming: vec![(short_block, short_result), (right_end, right_result)],
            ty: Type::Boolean,
            span,
        });
        Ok(destination)
        ...
        match header {
            ...
            ForHeader::Each {
                ...
                self.emit(Instruction::Constant {
                    ...
                    ty: Type::Integer(crate::types::IntegerType::Int32),
                    span,
                });
                ...
                let next_block = self.block();
                let exit = self.block();
                let next_index = self.value();
                self.terminate(Terminator::Jump {
                    target: condition_block,
                });
                self.current = condition_block;
                let loop_index = self.value();
                let condition = self.value();
                self.emit(Instruction::Binary {
                    destination: condition,
                    operator: "Less".into(),
                ...
                self.jump_if_open(next_block);
                self.current = next_block;
                let one = self.integer_one(Type::Integer(crate::types::IntegerType::Int32), span);
                self.emit(Instruction::Binary {
                ...
                let mut incoming = vec![(preheader, index)];
                if has_backedge {
                    incoming.push((next_block, next_index));
                }
                self.blocks[condition_block.0 as usize].instructions.insert(
                    0,
                    Instruction::Phi {
                        destination: loop_index,
                        incoming,
                        ty: Type::Integer(crate::types::IntegerType::Int32),
                        span,
                    },
