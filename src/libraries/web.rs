...
impl Provider for WebProvider {
    fn call(
        ...
        crate::tls::install_ring_provider().map_err(|message| {
            runtime_error(
                crate::diagnostic::DiagId::TLS_PROVIDER_UNAVAILABLE,
                message,
                span,
            )
        })?;
...
pub(crate) fn execute_callback(
    ...
    run_isolated(module, host, |core| {
        let span = crate::runtime::default_span();
            ...
            match method {
                ...
                "Route" => {
                    ...
                    let mut state = state.lock().map_err(|_| {
                        runtime_error(
                            crate::diagnostic::DiagId::SERVER_STATE,
                ...
                "Start" | "StartWithOptions" => {
                    ...
                    options.validate().map_err(|message| {
                        runtime_error(crate::diagnostic::DiagId::INVALID_OPTIONS, message, span)
                    })?;
                    let endpoint = net_endpoint(&arguments[1], span)?;
                    let listener =
                        crate::net::TcpListener::bind_with_backlog(endpoint, options.backlog)
                            .map_err(|error| {
                                runtime_error(
                                    crate::diagnostic::DiagId::WEB_LISTEN,
                                    error.to_string(),
                                    span,
                                )
                            })?;
                    ...
                    state
                        ...
                        .map_err(|message| {
                            runtime_error(crate::diagnostic::DiagId::WEB_LISTEN, message, span)
                        })?;
                    Ok(Value::Null)
                }
                    ...
                    match selected {
                        Ok(pattern) => {
                            ...
                            let handler = self
                                ...
                                .ok_or_else(|| {
                                    runtime_error(
                                        crate::diagnostic::DiagId::HANDLER_NOT_FOUND,
    ...
    fn web_state_call(
        ...
        if name.contains(".SessionStore.") {
            if method == "New" {
                ...
                .map_err(|message| {
                    runtime_error(crate::diagnostic::DiagId::SESSION_CONFIG, message, span)
                })?;
            ...
            if method == "CONSTRUCTOR" {
                ...
                let scraper = crate::web_state::Scraper::parse(html).map_err(|message| {
                    runtime_error(crate::diagnostic::DiagId::SCRAPER_INPUT, message, span)
                })?;
                self.scrapers.insert(*handle, scraper);
                return Ok(Value::Null);
            }
            ...
            if method == "New" {
                ...
                let max_redirects =
                    usize::try_from(integer(&arguments[3], span)?.0).map_err(|_| {
                        runtime_error(
                            crate::diagnostic::DiagId::INVALID_EGRESS_POLICY,
            ...
            let make_options = |offset: usize| -> Result<crate::web::ServerOptions, Diagnostic> {
                let value = |index: usize| {
                    usize::try_from(integer(&arguments[offset + index], span)?.0).map_err(|_| {
                        runtime_error(
                            crate::diagnostic::DiagId::INVALID_OPTIONS,
    ...
    fn web_request_call(
        ...
        if name.contains(".Request.") {
            if method == "CONSTRUCTOR" {
                ...
                .map_err(|message| {
                    runtime_error(crate::diagnostic::DiagId::REQUEST_INVALID, message, span)
                })?;
            ...
            match method {
                ...
                "Header" => {
                    ...
                    response
                        ...
                        .ok_or_else(|| {
                            runtime_error(
                                crate::diagnostic::DiagId::HEADER_NOT_FOUND,
