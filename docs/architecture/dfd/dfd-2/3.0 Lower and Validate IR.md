...
**3.0 Lower and Validate IR** turns Analyze products into the **one BN IR** that both backends consume.

1. **3.1 Lower** reads AST + semantic model and builds draft BN IR in **D2**.
2. **3.2 Validate** checks structural / light semantic constraints on that IR, writes the validated form back to **D2**, and records IR diagnostics in **D3**.
