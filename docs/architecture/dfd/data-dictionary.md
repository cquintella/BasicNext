...
- **`diagnostics`**: structured errors and warnings from analysis, IR validation, **target support (`validate_for`)**, runtime, or compile/link — with stable codes, source locations (spans), and (in the 0.4.5 expressive-diagnostics target) title/message/causes/help. **Support** failures use `TARGET_UNSUPPORTED_*` and must not be labeled as language/`INVALID_IR_*`. This is what the human reads to fix the program. The same diagnostic *facts* may also be mirrored into the process log; they must not contradict each other.
