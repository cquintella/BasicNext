...
| **GC-PAR** | Parity beyond stdout | For **each feature/op claimed supported** on a target, run the **pertinent** families (numeric boundaries, errors, observable effects, objects, opt stability) — not a single global “one family somewhere.” Floor for early buckets may stage families, but **announced support ⇒ matching families** — [conformance.md](conformance.md). A HOST capability counts as **shared by construction** when its core has one definition (`rg -l` → one file under `bn_rt`/`bn_host_exec`) and parity fixtures verify the *shells* (0.5.2a) | Happy-path stdout/exit only; “one extra family anywhere” while claiming a broad subset; two copies kept equal by vigilance |
...
| **GC-POL** | Execution policy | Requirements ≠ support ≠ policy; compiled path has policy carrier + **`bn_rt` re-check** at call boundary for the claimed HOST subset; **one policy type** (`bn_rt::Policy`) held by `HostEnv` and by the native image's single static, **one env parser** (`Policy::narrow_from_env`), malformed input fail-closed on both backends — `tests/check-shared-cores.sh` (0.5.2a) | Build-time allowlist only; two hand-mirrored policy readers |
