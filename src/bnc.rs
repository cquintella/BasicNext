...
fn source_less_diagnostic(code: bn_diag::DiagId, detail: impl Into<String>) -> String {
    bn_diag::Diagnostic::structured_source_less(code, vec![("detail".into(), detail.into().into())])
        .map_or_else(
...
fn main() -> ExitCode {
    ...
    let options = match parse_args(all_args) {
        ...
        Err(err) => {
            eprintln!(
                "error[BNC]: {}",
                source_less_diagnostic(bn_diag::DiagId::BNC, err)
            );
            return usage();
        }
    };
