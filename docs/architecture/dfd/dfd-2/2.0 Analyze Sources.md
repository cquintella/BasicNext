...
**Module resolution:** ordered search path — [`module-path.md`](../../module-path.md).
...
| Flow | From | Meaning |
| --- | --- | --- |
| A01 | 1.0 Control | Pipeline schedule for this job: entry path, resolved **programs-dir** + ordered **module-path**, profile note that Frontend must run. |
...
1. **2.1 Load** resolves the entry against **programs-dir** / entry path, then resolves later `IMPORT`s by walking the ordered **module-path** (first hit wins; stdlib `modules/bn` included per defaults) and deposits texts in **D1**.
...
5. **2.5 Semantic analysis** builds the semantic/symbol model (**D_sym**) over the graph/AST. It **must** implement the checklist in [`semantic-analysis.md`](../../semantic-analysis.md): definitions available by path, operand compatibility, boolean conditions, call/return signatures, and valid references.
...
| 2.5 | Semantic analysis (symbols / types) — see [`semantic-analysis.md`](../../semantic-analysis.md) |
...
Analyze operates on **session snapshots** `(SourceId, Revision, text)`, including unsaved buffers when driven by LSP — [frontend-session.md](../../frontend-session.md).
