...
use crate::web::{RouteOutcome, ServerState};
use http_body_util::Full;
use hyper::{Request, StatusCode, body::Bytes};
use hyper_util::rt::{TokioExecutor, TokioIo};

struct ScriptedResolver {
    answers: HashMap<String, Vec<bn_host_net::net::Address>>,
}
...
impl super::AddressResolver for ScriptedResolver {
    fn resolve(
        ...
        _port: u16,
        maximum: usize,
    ) -> Result<Vec<bn_host_net::net::Address>, String> {
        self.answers
            .get(host)
...
fn client_rejects_oversized_request_before_transport() {
    let body = "x".repeat(bn_limits::web_limits().max_body_bytes + 1);
    ...
    let resolver = ScriptedResolver {
        answers: HashMap::from([
            (
                "mixed.test".into(),
                vec![
                    bn_host_net::net::Address::parse("8.8.8.8").unwrap(),
                    bn_host_net::net::Address::parse("::ffff:127.0.0.1").unwrap(),
                ],
            ),
            (
                "public.test".into(),
                vec![bn_host_net::net::Address::parse("8.8.8.8").unwrap()],
            ),
            (
                "blocked.test".into(),
                vec![bn_host_net::net::Address::parse("100.64.0.1").unwrap()],
            ),
        ]),
    };
...
fn explicit_egress_policy_is_applied_after_resolution() {
    let resolver = ScriptedResolver {
        answers: HashMap::from([(
            "allowed.test".into(),
            vec![bn_host_net::net::Address::parse("93.184.216.34").unwrap()],
        )]),
    };
    let policy = crate::web::EgressPolicy::new(
        Some(vec!["http".into()]),
        Some(vec![
            bn_host_net::net::Cidr::parse("93.184.216.0/24").unwrap(),
        ]),
        Some(vec![80]),
        2,
        1000,
...
fn egress_policy_multi_a_first_out_of_cidr_later_allowlisted() {
    ...
    let resolver = ScriptedResolver {
        answers: HashMap::from([(
            "multi-a.test".into(),
            vec![
                bn_host_net::net::Address::parse("1.1.1.1").unwrap(), // out of CIDR
                bn_host_net::net::Address::parse("93.184.216.34").unwrap(), // in CIDR
            ],
        )]),
    };
    let policy = crate::web::EgressPolicy::new(
...
fn egress_policy_multi_a_none_allowlisted_fails_closed() {
    ...
    let resolver = ScriptedResolver {
        answers: HashMap::from([(
            "multi-c.test".into(),
            vec![
                bn_host_net::net::Address::parse("1.1.1.1").unwrap(),
                bn_host_net::net::Address::parse("2.2.2.2").unwrap(),
            ],
        )]),
    };
    let policy = crate::web::EgressPolicy::new(
...
fn redirect_destination_is_revalidated_with_the_same_resolver_policy() {
    let resolver = ScriptedResolver {
        answers: HashMap::from([
            ...
            (
                "blocked.test".into(),
                vec![bn_host_net::net::Address::parse("169.254.169.254").unwrap()],
            ),
        ]),
    };
    let redirect =
