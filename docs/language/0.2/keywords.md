...
word cannot be an identifier: it may have 0.1 or 0.2 syntax or be deliberately
...
must not coexist without a deliberate decision. `Accepted syntax` is 0.2 unless
...
| `HOST` | Names the environment that provides capabilities. Capability names after `HOST.` start with a capital letter (`HOST.Args`, `HOST.Clock`, `HOST.Console`, `HOST.Random`, `HOST.FileSystem`; later `HOST.Network`). `HOST.Main` is withdrawn in 0.2. | Reserved |
