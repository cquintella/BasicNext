...
fn server_admission_bounds_connections_before_worker_spawn() {
    let mut server = ServerState::new();
    server.start().unwrap();
    for _ in 0..bn_limits::web_limits().active_connections {
        server.admit_connection().unwrap();
    }
    assert_eq!(
...
fn server_stop_cancels_an_admitted_socket_without_sleep() {
    ...
    let accepted = bn_host_net::net::TcpStream::from_std(accepted);
    let mut server = ServerState::new();
    server.start().unwrap();
    server.admit_connection().unwrap();
...
fn server_stop_cancels_multiple_admitted_sockets() {
    ...
    for _ in 0..2 {
        ...
        accepted.push(bn_host_net::net::TcpStream::from_std(
...
fn egress_policy_is_immutable_and_fail_closed() {
    ...
    let too_many = std::iter::repeat_n("http", bn_limits::web_limits().egress_list_max + 1)
        .collect::<Vec<_>>()
        .join(",");
